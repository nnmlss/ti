...
export function SiteDetailView({ site }: SiteDetailViewProps) {
  const renderLocalizedText = (text: { bg?: string; en?: string } | undefined, label: string) => {
        ...
        <Grid container spacing={2}>
          {hasBg && (
            <Grid size={{ xs: 12, md: 6 }}>
              <Typography variant="subtitle2" gutterBottom>Bulgarian:</Typography>
              {data!.bg!.filter(item => item.trim()).map((item, index) => (
          ...
          {hasEn && (
            <Grid size={{ xs: 12, md: 6 }}>
              <Typography variant="subtitle2" gutterBottom>English:</Typography>
              {data!.en!.filter(item => item.trim()).map((item, index) => (
                <Typography key={index} variant="body2" sx={{ mb: 1 }}>
                  • {item}
                </Typography>
              ))}
  ...
  const renderLandingFields = () => {
    ...
    const hasLandingFields = site.landingFields.some(field => 
      field.description?.bg || field.description?.en || 
      (field.location?.coordinates && field.location.coordinates.some(coord => coord !== null))
    );
    ...
    return (
      <Box sx={{ mb: 3 }}>
        ...
        {site.landingFields.map((field, index) => {
          const hasContent = field.description?.bg || field.description?.en || 
                           (field.location?.coordinates && field.location.coordinates.some(coord => coord !== null));
              ...
              {field.location?.coordinates && field.location.coordinates.some(coord => coord !== null) && (
                <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
                  <Typography variant="body2">
                    Coordinates: {field.location.coordinates[1]}, {field.location.coordinates[0]}
                  </Typography>
                  <Button
                    size="small"
  ...
  return (
    <Card sx={{ maxWidth: 800, mx: 'auto' }}>
      <CardContent sx={{ p: 4 }}>
        ...
        {renderLocalizedText(site.access, "Access Description")}
        {renderBilingualArray(site.accomodations, "Accommodations")}
        {renderBilingualArray(site.alternatives, "Alternatives")}
