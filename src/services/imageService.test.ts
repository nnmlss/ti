...
vi.mock('../utils/fileUtils.js', () => ({
  getGalleryPath: vi.fn((...segments: string[]) => 
    `/mock/gallery/${segments.join('/')}`
  ),
}));
...
describe('imageService', () => {
  ...
  describe('generateThumbnailsForImage', () => {
    ...
    it('should throw error if original file not found', async () => {
      ...
      await expect(generateThumbnailsForImage(filename)).rejects.toThrow('Original image file not found');
    });
